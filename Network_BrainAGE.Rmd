...
# Load Template Excel file for Network BrainAGE input
...
demographics_df <- read_csv("../data/raw/study-HBN_desc-participants.tsv")
```
...
#Subject IDs in demographics data
...
gendered_data <- demographics_df %>%
  filter(time_point == "01") %>%  # Filter for the desired time_point (baseline)
  filter(assigned_sex_at_birth == sex) %>%  # Filter for sex
...
# Add the female subject_ID, age, and assigned sex to the template_df
